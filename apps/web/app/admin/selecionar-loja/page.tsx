...
import { Building, AlertTriangle, Power, Palette } from "lucide-react";
...
// Função igual da loja/page pra aplicar tema
const applyTheme = (theme: string, cardStyle: string, cardSize: string, font: string) => {
    ...
    root.style.setProperty('--cor-primaria', '#10b981'); // verde igual da loja
...
export default function SelectLojaPage() {
    ...
    useEffect(() => {
        ...
        // Pega preferências do cookie e aplica
    ...
    const handleSaveTheme = (data: { theme?: string, card_style?: string, card_size?: string, font_family?: string }) => {
        const newTheme = data.theme || theme;
        const newCardStyle = data.card_style || cardStyle;
        const newCardSize = data.card_size || cardSize;
        const newFont = data.font_family || font;
        ...
        setCookie("card_style", newCardStyle);
        setCookie("card_size", newCardSize);
        setCookie("font_family", newFont);

        applyTheme(newTheme, newCardStyle, newCardSize, newFont);
    }
            ...
            <div className="w-full max-w-7xl mx-auto px-3 sm:px-4 md:px-6 py-3 sm:py-6">
                ...
                >
                    <div className="flex items-center gap-3">
                        <Building size={32} style={{ color: 'var(--cor-primaria)' }} />
                        ...
                        >
                            <div className="flex items-start justify-between mb-4">
                                ...
                                <div className="flex flex-col gap-2 items-end">
                                    <span className={`px-3 py-1 rounded-full text-xs font-semibold ${loja.is_active ? "bg-green-600 text-white" : "bg-red-600 text-white"
                                        }`}>
