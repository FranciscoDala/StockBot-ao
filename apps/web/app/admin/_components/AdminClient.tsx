...
import { Store, Users, Plus, Edit, Eye, User, Loader2, Trash2, Mail, MapPin, TrendingUp, AlertCircle } from "lucide-react";
...
export type ModoLoja = "venda" | "cliente" | "completo" // <- ADICIONADO
...
const emptyForm: FormData = {
    ...
    is_active: true,
    endereco: "",
    modo: "completo", // <- ADICIONADO DEFAULT
    modoDono: 'novo',
    dono_existente_id: "",
...
export default function AdminClient({ lojasIniciais, donosIniciais }: { lojasIniciais: Loja[], donosIniciais: Dono[] }) {
    ...
    const [confirmErrorModalOpen, setConfirmErrorModalOpen] = useState(false);
        ...
        if (loja) {
            ...
            setFormData({
               ...emptyForm,
                ...
                modo: data.modo || "completo", // <- ADICIONADO
                modoDono: 'existente',
    ...
    const handleConfirmSave = async () => {
        ...
        try {
            ...
            if (res.status === 401) return handleTerminarSessao();
            if (res.status === 403) {
                setConfirmErrorMessage("Senha do ADMIN incorreta");
                setConfirmErrorModalOpen(true);
                setIsSaving(false);
                return;
            }
    ...
    const handleDeleteLoja = async () => {
        ...
        try {
            ...
            if (res.status === 401) return handleTerminarSessao();
            if (res.status === 403) {
                setDeleteErrorMessage("Senha do ADMIN incorreta");
                setDeleteErrorModalOpen(true);
                setIsDeleting(false);
                return;
            }
