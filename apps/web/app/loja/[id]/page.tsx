...
export default function LojaPage() {
    ...
    const [menuMobileOpen, setMenuMobileOpen] = useState(false); // <- NOVO
    ...
    const executarAcaoComSenha = async (senha_dono: string) => {
        if (!acaoPendente ||!token) return; setSaving(true); setShowPermissaoModal(false); const { tipo, entidade, data } = acaoPendente; try {
            if (entidade === 'user') {
                ...
                if (tipo === 'editar' && data) {
                    const payload: any = {...formDataUser, senha_dono: senha_dono };
                    if (formDataUser.senha) { payload.senha = formDataUser.senha; }
                    await fetchComAuth(`${API_URL}/lojas/id/${lojaId}/usuarios/${(data as UsuarioLojaPage).id}`, token, { method: "PUT", body: JSON.stringify(payload) });
    ...
    return <>
        ...
        {activeTab === "venda"? <div className="fixed inset-0 z-40 bg-black"><VendaTab {...{ produtos, carrinho, busca, setBusca, formaPagamento, setFormaPagamento, valorRecebido, setValorRecebido, subtotal, totalItens, troco, podeFinalizar, adicionarAoCarrinho, confirmarRemoverItem, handleFinalizar, showConfirmarModal, setShowConfirmarModal, itemParaRemover, handleConfirmarRemocao, showConfirmarFinalizar, setShowConfirmarFinalizar, executarFinalizarVenda, loadingVenda, formatCurrency, onClose: () => { setActiveTab(initialTabs[0].id); setCarrinho([]) }, token, lojaId, nomeLoja: loja?.nome || "PDV" }} /></div> :
            <div className="min-h-screen bg-black text-white">
                <div className="w-full max-w-7xl mx-auto px-3 sm:px-4 md:px-6 py-3 sm:py-6"> {/* <- WRAPPER RESPONSIVO */}
                    ...
                    <div className="flex items-center justify-between gap-3 mb-4 sm:mb-6">
                        <div className="flex items-center gap-2 sm:gap-3 min-w-0 flex-1">
                            ...
                            <div className="min-w-0">
                                ...
                                <p className="text-[10px] sm:text-xs text-gray-400 truncate">{loja?.endereco}</p>
                            </div>
                        </div>
                        <div className="flex items-center gap-2 shrink-0">
                            <span className={`px-2 sm:px-3 py-1 rounded-full text-[10px] sm:text-xs font-medium ${loja?.is_active? "bg-green-600" : "bg-gray-600"}`}>{loja?.is_active? "ativa" : "inativa"}</span>
                            <button onClick={handleSair} className="px-2.5 sm:px-3 py-1.5 sm:py-2 bg-red-600 rounded-lg text-[10px] sm:text-xs font-bold flex items-center gap-1.5 hover:bg-red-700 transition">
                ...
                <ConfirmarModal open={showConfirmarModal} onClose={() => { setShowConfirmarModal(false); setAcaoPendente(null) }} onConfirm={executarAcaoComSenha} titulo="Confirmar Ação" descricao={acaoPendente?.descricao || ""} loading={saving} />
            </div>
        }
