...
export default function LojaPage() {
    ...
    const executarAcaoComSenha = async (senha_dono: string) => {
        ...
        try {
            if (entidade === 'user') {
                if (tipo === 'adicionar') {
                    await fetchComAuth(`${API_URL}/lojas/id/${lojaId}/usuarios`, token, { method: "POST", body: JSON.stringify({...formDataUser, senha: formDataUser.senha, senha_confirmacao: formDataUser.senha, senha_dono: senha_dono }) });
                    await fetchEquipa(token);
                    toast.success("Membro adicionado!");
                }

                if (tipo === 'editar' && data) {
                    const payload: any = { senha_dono: senha_dono }; // <- SEMPRE MANDA
                ...
                if (tipo === 'apagar' && data) {
                    await fetchComAuth(`${API_URL}/lojas/id/${lojaId}/usuarios/${(data as UsuarioLojaPage).id}`, token, { method: "DELETE", body: JSON.stringify({ senha_dono: senha_dono }) });
                    await fetchEquipa(token);
