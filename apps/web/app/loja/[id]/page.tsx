...
const fetchEquipa = async (currentToken: string) => { if (!currentToken ||!lojaId) return; try { const data = await fetchComAuth(`${API_URL}/lojas/id/${lojaId}/usuarios`, currentToken); setEquipa(Array.isArray(data)? data : []); } catch (e) { setEquipa([]) } };
...
useEffect(() => { if (!token ||!lojaId) return; const WS_URL = process.env.NEXT_PUBLIC_WS_URL; const socket = new WebSocket(`${WS_URL}/ws/lojas/${lojaId}?token=${token}`); socket.onopen = () => setWs(socket);
    socket.onmessage = (event) => { const data = JSON.parse(event.data); if (data.tipo === "stock.updated") {
