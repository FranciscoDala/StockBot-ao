...
import { EstatisticasTab } from "./_components/tabs/EstatisticasTab"; // <- NOVO
                ...
                <div className="w-full max-w-7xl mx-auto px-3 sm:px-4 md:px-6 py-3 sm:py-6">
                    ...
                    <div className="flex items-center justify-between gap-3 mb-4 sm:mb-6">
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                            <span className={`px-2 sm:px-3 py-1 rounded-full text-xs sm:text-xs font-medium ${loja?.is_active? "bg-green-600" : "bg-gray-600"}`}>{loja?.is_active? "ativa" : "inativa"}</span>
                            <button onClick={handleSair} className="px-2.5 sm:px-3 py-1.5 sm:py-2 bg-red-600 rounded-lg text-xs sm:text-xs font-bold flex items-center gap-1.5 hover:bg-red-700 transition">
                    ...
                    <div className="pb-8">
                        ...
                        {activeTab === "estatisticas" && <EstatisticasTab lojaId={lojaId} token={token} formatCurrency={formatCurrency} />} // <- NOVO
                        {!["dados", "venda", "produtos", "equipa", "estatisticas"].includes(activeTab) && <div className="bg-neutral-900 p-4 sm:p-6 rounded-xl text-center text-gray-400 text-sm">Em breve: {allTabs.find(t => t.id === activeTab)?.label}</div>}
                    </div>
                </div>
