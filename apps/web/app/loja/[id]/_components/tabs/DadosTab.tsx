...
export function DadosTab({
    loja,
    user,
    lojaId // <- ADICIONEI SÓ PRA FICAR PADRONIZADO
}: {
    loja: Loja | null | undefined;
    user: userread | null;
    lojaId?: string; // <- OPCIONAL
}) {
    return (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6">
            <div className="bg-neutral-900 p-4 sm:p-6 rounded-xl border-neutral-800">
                ...
                <p className="text-xs sm:text-sm text-gray-400">ID: <span className="text-white break-all">{lojaId || "-"}</span></p> {/* <- TROQUEI SLUG POR ID */}
                <p className="text-xs sm:text-sm text-gray-400 mt-2">Slug: <span className="text-white break-all">{loja?.slug || "-"}</span></p>
                <p className="text-xs sm:text-sm text-gray-400 mt-2">NIF: <span className="text-white">{loja?.nif || "-"}</span></p>
            ...
            <div className="bg-neutral-900 p-4 sm:p-6 rounded-xl border-neutral-800">
                ...
                <p className="text-xs sm:text-sm text-gray-400 mt-2">Tel: <span className="text-white">{loja?.telefone || "-"}</span></p>
            </div>
        </div>
    )
}
