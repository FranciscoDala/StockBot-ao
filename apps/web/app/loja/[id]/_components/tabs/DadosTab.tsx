    ...
    const carregarDados = useCallback(async () => {
        ...
        try {
            ...
            if (resResumo.ok) {
                ...
                console.log("STATUS CAIXA:", resumo.status);
            } else {
                setIsCaixaAberto(false);
                console.log("ERRO STATUS CAIXA:", resResumo.status);
            }
            ...
            const [resVendas, resSaidas, resMov, resumoMesData] = await Promise.all([
                ...
                fetch(`${API_URL}/caixas/historico?loja_id=${lojaId}&data=${hojeStr}`, { headers: { "Authorization": `Bearer ${token}` } }),
                carregarResumoMes()
            ]);
            ...
            if (movsHoje.length > 0 && movsHoje.some(m => m.forma_pagamento)) {
                ...
                console.log("USANDO FALLBACK: Vendas de Hoje");
                cashHoje = vendasHoje
