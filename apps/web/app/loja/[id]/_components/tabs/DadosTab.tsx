...
export function DadosTab({ loja, user, lojaId: lojaIdProp, token: tokenProp, theme, cardStyle, cardSize, onCaixaMudou }: Props) {
    ...
    const carregarDados = useCallback(async () => { // <- JUNTEI KPI + STATUS CAIXA + MOV CAIXA
        ...
        try {
            ...
            // 5. Calcula Movimentacoes do Caixa de hoje - AJUSTE AQUI
            ...
            // NOVO: Separa Cash e TPA igual no CaixaModal
            const cashHoje = movsHoje
              .filter(m => tiposEntrada.includes(m.tipo) && m.forma_pagamento === 'Dinheiro')
              .reduce((acc, m) => acc + Number(m.valor || 0), 0);

            const tpaHoje = movsHoje
              .filter(m => tiposEntrada.includes(m.tipo) && m.forma_pagamento && ['TPA', 'Transferencia'].includes(m.forma_pagamento))
              .reduce((acc, m) => acc + Number(m.valor || 0), 0);
            ...
            setKpis({
                vendaDiaria: cashHoje + tpaHoje, // <- AJUSTE: agora pega do caixa e não da tabela vendas
