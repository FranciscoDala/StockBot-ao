...
export function DocumentosTab({ dadosFiltrados, tipoRelatorio, loja }: DocumentosTabProps) {
    ...
    const vendas = dadosFiltrados?.vendasF || []
    ...
    const exportarWord = async () => {
        ...
        try {
            ...
            children.push(
                ...
                new Paragraph({ children: [new TextRun({ text: `Relatório: ${tipoRelatorio}`, bold: true, size: 24 })], heading: HeadingLevel.HEADING_2, alignment: AlignmentType.CENTER }),
                new Paragraph({ text: `Emitido em: ${dataHoje}`, alignment: AlignmentType.CENTER }),
                new Paragraph({ children: [new TextRun({ text: `Total Geral: ${formatarKZ(totalGeral)}`, bold: true })] }),
                new Paragraph({ text: "" })
            )
