...
export function ClientesTab({ lojaId, token, theme, cardStyle, cardSize, formatCurrency }: Props) {
    ...
    const handleConfirmarPagamento = async () => { // 3. FUNÇÃO AJUSTADA
        if (!token ||!clienteSelecionado ||!vendaSelecionada ||!valorPagamento || parseFloat(valorPagamento) <= 0) {
            return toast.error("Valor inválido");
        }
        setSavingPagamento(true); // usa loading separado
        try {
            ...
            setShowPagarModal(false); // fecha só a modal de pagar
            setVendaSelecionada(null);

            // recarrega em background sem fechar a modal principal
            await Promise.all([fetchClientes(), fetchDetalhesCliente(clienteSelecionado)]);
