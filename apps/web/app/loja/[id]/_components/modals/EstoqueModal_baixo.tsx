...
export function EstoqueModalBaixo({ open, onOpenChange, produtos }: Props) {
          ...
          <DialogHeader className="p-4 sm:p-6 border-b shrink-0 flex-row items-center justify-between" style={{ borderColor: 'color-mix(in srgb, var(--cor-borda) 20%, transparent)', backgroundColor: 'var(--cor-card)' }}>
            <div>
              ...
              <DialogDescription className="text-xs sm:text-sm" style={{ color: 'var(--cor-texto-sec)' }}>
                {produtos.length} produto(s) abaixo do estoque mínimo
              </DialogDescription>
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 sm:p-6">
            {produtos.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full gap-3">
                <p className="font-semibold">Nenhum produto com estoque baixo</p>
                  ...
                  <div className="flex w-max gap-4">
                    {produtos.map(prod => (
                      <div
                        ...
                        style={{ background: 'var(--cor-card)', borderColor: 'color-mix(in srgb, var(--cor-borda) 20%, transparent)' }}
                      >
                        <p className="font-bold truncate mb-2" style={{ color: 'var(--cor-texto)' }}>{prod.nome}</p>
                        <div className="space-y-1 text-sm">
                          <p style={{ color: 'var(--cor-texto-sec)' }}>Estoque Atual: <span className="font-bold text-yellow-500">{prod.estoque}</span></p>
                          <p style={{ color: 'var(--cor-texto-sec)' }}>Mínimo: <span className="font-semibold">{prod.estoque_minimo}</span></p>
                        </div>
