...
export function DetalhesClienteModal({ open, onClose, cliente, vendas, produtos, onPagar, onSalvarFiado, formatCurrency, loading }: Props) {
    const [abaAtiva, setAbaAtiva] = useState<'dividas' | 'fiado'>('fiado'); // <- COMEÇA NA ABA FIADO
    ...
    useEffect(() => {
        if (open) {
            ...
            setAbaAtiva('fiado'); // <- abre direto no fiado
            setCarrinho([]);
            setBuscaProduto("");
                    ...
                    <div className="min-w-0 flex-1">
                        ...
                        <div className="flex items-center gap-4 mt-1 flex-wrap">
                            ...
                            <div className="flex items-center gap-1 text-sm">
                                ...
                                <span>Dívida: </span>
                                <span className="font-bold" style={{ color: '#ef4444' }}>{formatCurrency(cliente?.total_divida?? 0)}</span>
                            </div>
                            <div className="flex items-center gap-1 text-sm">
                                <Calendar size={14} style={{ color: 'var(--cor-texto-sec)' }} />
                                <span>Última: {cliente?.ultima_compra? new Date(cliente.ultima_compra).toLocaleDateString('pt-AO') : 'Nunca'}</span>
                            </div>
                        </div>
                    </div>
                        ...
                        <>
                            ...
                            {abaAtiva === 'dividas' && (
                                <div className="h-full overflow-y-auto p-4 sm:p-6 space-y-4">
                                    ...
                                    {vendas.length === 0? (
                                        <div className="flex flex-col items-center justify-center gap-3 rounded-xl border-2 border-dashed p-10 text-center mt-4" style={{ borderColor: 'var(--cor-borda)', background: 'var(--cor-card)' }}>
                                ...
                                <div className="h-full grid grid-cols-1 lg:grid-cols-5 gap-0">
                                    ...
                                    <div className="lg:col-span-3 p-4 sm:p-6 border-r flex flex-col" style={{ borderColor: 'var(--cor-borda)' }}>
                                        ...
                                        <div className="relative mb-4 shrink-0">
                                            ...
                                            <Input placeholder="Buscar produto por nome..." value={buscaProduto} onChange={e => setBuscaProduto(e.target.value)} className="pl-9 h-10" />
                                        </div>
                                            ...
                                            {produtosFiltrados.map(p => (
                                                <div key={p.id} className="flex items-center justify-between p-3 border rounded-lg transition hover:border-[var(--cor-primaria)]" style={{ borderColor: 'var(--cor-borda)', background: 'var(--cor-card)', opacity: (p.estoque?? 0) <= 0? 0.5 : 1 }}>
                                                    <div className="min-w-0 flex-1">
                                                        ...
                                                        <p className="text-xs" style={{ color: 'var(--cor-texto-sec)' }}>{formatCurrency(p.preco)} | Estoque: {p.estoque?? 0}</p>
                                                    </div>
                                    ...
                                    <div className="lg:col-span-2 p-4 sm:p-6 flex-col" style={{ backgroundColor: 'var(--cor-card)' }}>
                                            ...
                                            {carrinho.map(i => (
                                                <div key={i.id} className="flex items-center justify-between p-3 rounded-lg" style={{ background: 'color-mix(in srgb, var(--cor-primaria) 5%, transparent)' }}>
                                                    <div className="min-w-0 flex-1">
                                                        ...
                                                        <p className="text-xs" style={{ color: 'var(--cor-texto-sec)' }}>{formatCurrency(i.preco)} x {i.qtd} = {formatCurrency(i.preco * i.qtd)}</p>
                                                    </div>
                                        ...
                                        <div className="border-t pt-4 mt-4 shrink-0" style={{ borderColor: 'var(--cor-borda)' }}>
                                            <div className="flex justify-between items-center mb-4">
                                                <p className="font-semibold">Total da Dívida:</p>
                                                <p className="font-bold text-2xl" style={{ color: 'var(--cor-primaria)' }}>{formatCurrency(totalCarrinho)}</p>
                                            </div>
