...
export function DetalhesClienteModal({ open, onClose, cliente, vendas, produtos, onPagar, onSalvarFiado, formatCurrency, loading, cardStyle = 'arredondado' }: Props) {
    ...
    }, [abaAtiva, podeFinalizar, onClose]);
                    ...
                    <div className="flex-1 min-h-0">
                        {loading? (
                            ...
                            <>
                                ...
                                {abaAtiva === 'fiado' && (
                                    <div className="h-full flex flex-col lg:grid lg:grid-cols-3">
                                        {/* COLUNA PRODUTOS - IGUAL VENDA */}
                                        <div className="lg:col-span-2 p-3 flex flex-col overflow-hidden">
                                                    ...
                                                    return (
                                                        ...
                                                        >
                                                            <div className="relative w-full aspect-square" style={{ backgroundColor: 'var(--cor-fundo)' }}>
                                                                {p.imagem_url? <img src={p.imagem_url.startsWith('http')? p.imagem_url : `${API_BASE}${p.imagem_url}`} alt={p.nome} className="w-full h-full object-cover" />
                                                        ...
                                                        return (
                                                            <div key={i.id} className="flex items-center justify-between p-2 rounded-md" style={{ background: 'var(--cor-fundo)', borderRadius: radius }}>
                                                                <div className="min-w-0 flex-1">
                                                                    <p className="text-xs font-semibold truncate">{i.nome}</p>
                                                                    <p className="text-xs font-bold" style={{ color: 'var(--cor-primaria)' }}>{formatCurrency(preco)} x {i.qtd}</p>
                                                                </div>
                                                                <div className="flex items-center gap-1">
                                                                    <Button size="icon" variant="outline" className="h-7 w-7" onClick={() => alterarQtd(i.id, -1)}><Minus size={12} /></Button>
                                                                    <Button size="icon" variant="outline" className="h-7 w-7" onClick={() => removerDoCarrinho(i.id)}><Trash2 size={12} /></Button>
                                                                </div>
                                                            </div>
                                                        )
                                                    })}
                                            ...
                                            <div className="lg:hidden py-3 space-y-2 border-t sticky bottom-0" style={{ backgroundColor: 'var(--cor-card)', borderColor: 'var(--cor-primaria)30' }}>
                                                <div className="flex justify-between items-center">
                                                    <span className="text-xs opacity-70">Total da Dívida</span>
                                                    <span className="font-bold text-lg" style={{ color: 'var(--cor-primaria)' }}>{formatCurrency(totalCarrinho)}</span>
                                                </div>
                                                <Button onClick={handleSalvarFiado} disabled={!podeFinalizar} className="w-full h-12 text-base font-bold" style={{ background: 'var(--cor-primaria)', color: '#fff', borderRadius: radius }}>
                                                    {salvando? <Loader2 size={18} className="animate-spin" /> : "Confirmar e Salvar Dívida"}
                                                </Button>
                                            </div>
                                        </div>

                                        {/* DESKTOP CARRINHO - DIREITA */}
                                        <div className="border-t lg:border-t-0 lg:border-l hidden lg:flex lg:flex-col h-[calc(100vh-140px)] sticky top-0" style={{ backgroundColor: 'var(--cor-card)', borderColor: 'var(--cor-primaria)30' }}>
                                                        ...
                                                        <div key={i.id} className="p-2.5 rounded-lg" style={{ backgroundColor: 'var(--cor-fundo)', borderRadius: radius }}>
                                                            <div className="flex justify-between items-start gap-2">
                                                                <div className="min-w-0">
                                                                    <p className="font-semibold text-xs truncate">{i.nome}</p>
                                                                    <p className="text-xs font-bold" style={{ color: 'var(--cor-primaria)' }}>{formatCurrency(preco)} x {i.qtd}</p>
                                                                </div>
                                                                <div className="flex gap-1">
                                                                    <Button size="icon" variant="outline" className="h-6 w-6" onClick={() => alterarQtd(i.id, -1)}><Minus size={10} /></Button>
                                                                    <Button size="icon" variant="outline" className="h-6 w-6" onClick={() => removerDoCarrinho(i.id)}><Trash2 size={10} /></Button>
                                                                </div>
                                                            </div>
                                            ...
                                            <div className="border-t p-3 space-y-2 mt-auto">
                                                ...
                                                <Button onClick={handleSalvarFiado} disabled={!podeFinalizar} className="w-full h-11 text-base font-bold" style={{ background: 'var(--cor-primaria)', color: '#fff', borderRadius: radius }}>
