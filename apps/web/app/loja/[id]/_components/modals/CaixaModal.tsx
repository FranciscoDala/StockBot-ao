...
export function CaixaModal({ open, onOpenChange, onSave, lojaId, token }: Props) {
    ...
    const [resumoMes, setResumoMes] = useState<number>(0); // <- NOVO STATE
    ...
    // 1. Carrega resumo + movimentacoes + resumo mes + controla scroll
    useEffect(() => {
    ...
    const carregarResumoMes = async () => { // <- NOVA FUNÇÃO
        if (!API_URL ||!lojaId ||!token) return;
        try {
            ...
            const data = await res.json();
            setResumoMes(data.saidas_30dias || 0);
        } catch (error) {
            console.error(error);
            setResumoMes(0);
        }
...
function TabButton({ label, icon, active, onClick }: any) {
    return (
        <button
            ...
            onMouseEnter={(e) => { if (!active) e.currentTarget.style.backgroundColor = 'color-mix(in srgb, var(--cor-primaria) 8%, transparent)' }}
...
function AbaResumo({ resumo, resumoMes, movimentacoes, isCaixaAberto, onAbrir, onSangria }: { resumo: CaixaResumo | null, resumoMes: number, movimentacoes: Movimentacao[], isCaixaAberto: boolean, onAbrir: () => void, onSangria: () => void }) {
    ...
    const statusConfig = isCaixaAberto? {
        cor: 'var(--cor-sucesso)', bg: 'color-mix(in srgb, var(--cor-sucesso) 8%, transparent)', border: 'color-mix(in srgb, var(--cor-sucesso) 25%, transparent)', icon: <CheckCircle size={22} />, titulo: 'Caixa Aberto', subtitulo: 'Operações liberadas. Registre vendas e sangrias.'
    } : {
        cor: 'var(--cor-erro)', bg: 'color-mix(in srgb, var(--cor-erro) 8%, transparent)', border: 'color-mix(in srgb, var(--cor-erro) 25%, transparent)', icon: <Lock size={22} />, titulo: 'Caixa Fechado', subtitulo: 'Abra o caixa para iniciar as operações do dia.'
    }

    const CardMetrica = ({ titulo, valor, cor, bg, border, icon }: any) => (
        <div className="w-full transition hover:scale-[1.02]" style={{ background: bg, padding: '18px', borderRadius: 'var(--radius)', border: `1.5px solid ${border}` }}>
            <div className="flex items-center justify-between mb-3">
                <p className="text-sm font-semibold" style={{ color: 'var(--cor-texto-sec)' }}>{titulo}</p>
                <div className="p-2 rounded-lg" style={{ background: 'color-mix(in srgb, var(--cor-fundo) 50%, transparent)', color: cor }}>{icon}</div>
            </div>
    ...
    return (
        <div className="space-y-4 pb-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                ...
                <CardMetrica titulo="Saídas 30 Dias" valor={resumoMes} icon={<TrendingDown size={18} />} cor="var(--cor-erro)" bg="color-mix(in srgb, var(--cor-erro) 6%, transparent)" border="color-mix(in srgb, var(--cor-erro) 20%, transparent)" /> {/* <- NOVO CARD */}
            </div>

            {/* GRID COM OS 2 CARDS PRINCIPAIS */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="p-5 rounded-xl flex-col sm:flex-row sm:items-center sm:justify-between gap-2" style={{ background: 'color-mix(in srgb, var(--cor-aviso) 8%, transparent)', border: '2px solid var(--cor-aviso)' }}>
                    <div>
...
function AbaMovimentacoes({ movimentacoes }: { movimentacoes: Movimentacao[] }) {
    ...
    const tiposEntrada = ['entrada', 'abertura', 'suprimento'];
    const tiposSaida = ['saida', 'sangria', 'fechamento', 'estorno'];
    const getIcon = (tipo: string) => { if (tiposEntrada.includes(tipo)) return <ArrowUpRight size={16} className="text-[var(--cor-sucesso)]" />; return <ArrowDownRight size={16} className="text-[var(--cor-erro)]" /> }
    const isEntrada = (tipo: string) => tiposEntrada.includes(tipo);
    const movimentacoesFiltradas = movimentacoes.filter(mov => { const dataMov = new Date(mov.created_at).toISOString().split('T')[0]; return dataMov === dataSelecionada; });

    if (movimentacoesFiltradas.length === 0) {
        return (
            <div className="flex flex-col h-full">
                <div className="sticky top-0 z-10 p-3 rounded-lg mb-4" style={{ background: 'var(--cor-card)', border: '1px solid color-mix(in srgb, var(--cor-borda) 20%, transparent)' }}>
                    <div className="flex flex-col sm:flex-row sm:items-center gap-2">
                        <div className="flex items-center gap-2"><Calendar size={18} style={{ color: 'var(--cor-primaria)' }} /><label className="text-sm font-semibold whitespace-nowrap">Ver histórico de:</label></div>
                        <Input type="date" value={dataSelecionada} onChange={(e) => setDataSelecionada(e.target.value)} className="w-full sm:w-auto h-9" />
                    </div>
                </div>
                <div className="flex flex-1 flex-col items-center justify-center gap-3 rounded-xl border border-dashed p-6 text-center" style={{ borderColor: 'var(--cor-borda)', background: 'var(--cor-card)' }}>
                    <Inbox size={32} style={{ color: 'var(--cor-texto-sec)' }} /><h3 className="font-semibold">Nenhuma movimentação nesta data</h3><p className="text-xs" style={{ color: 'var(--cor-texto-sec)' }}>Tente selecionar outra data</p>
                </div>
            </div>
        )
    }
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <p className="font-semibold text-sm flex items-center gap-2 flex-wrap">
                                        {mov.descricao}
                                        {mov.forma_pagamento && (
                                            <span className="text-[10px] font-bold px-2 py-0.5 rounded-full" style={{ background: mov.forma_pagamento === 'Dinheiro'? 'color-mix(in srgb, var(--cor-sucesso) 15%, transparent)' : 'color-mix(in srgb, var(--cor-primaria) 15%, transparent)', color: mov.forma_pagamento === 'Dinheiro'? 'var(--cor-sucesso)' : 'var(--cor-primaria)' }}>{mov.forma_pagamento}</span>
                                        )}
                                    </p>
                                    <p className="text-xs" style={{ color: 'var(--cor-texto-sec)' }}>{formatDateTime(mov.created_at)}</p>
                                </div>
                            </div>
                            <p className={`font-bold text-sm ${isEntrada(mov.tipo)? 'text-[var(--cor-sucesso)]' : 'text-[var(--cor-erro)]'}`}>{isEntrada(mov.tipo)? '+' : '-'} {formatCurrency(mov.valor)}</p>
                        </div>
                    ))}
                </div>
            </div>
