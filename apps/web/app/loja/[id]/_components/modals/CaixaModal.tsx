...
export function CaixaModal({ open, onOpenChange, lojaId, token }: Props) {
    ...
    useEffect(() => {
        if (open && lojaId && token &&!showSangriaModal &&!showAberturaModal) {
            carregarResumoCaixa();
            if (abaAtiva === 'movimentacoes') carregarHistoricoPorData(); // <- MUDOU
        }
        if (open) document.body.style.overflow = 'hidden';
        else document.body.style.overflow = 'unset';
        return () => { document.body.style.overflow = 'unset'; }
    }, [open, lojaId, token, showSangriaModal, showAberturaModal, abaAtiva, dataSelecionada]); // <- ADICIONEI dataSelecionada
