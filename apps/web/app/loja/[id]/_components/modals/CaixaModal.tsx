...
// ADICIONADO: fallback do token igual no DadosTab
...
export function CaixaModal({ open, onOpenChange, onSave, lojaId: lojaIdProp, token: tokenProp }: Props) { // <- AJUSTADO: renomeei props
    const lojaId = lojaIdProp
    const token = tokenProp || getCookie('token') // <- ADICIONADO: fallback do token
...
function AbaResumo({ resumo, resumoMes, movimentacoes, isCaixaAberto, onAbrir, onSangria }: { resumo: CaixaResumo | null, resumoMes: number, movimentacoes: Movimentacao[], isCaixaAberto: boolean, onAbrir: () => void, onSangria: () => void }) {
    ...
    // AJUSTADO: toLowerCase para aceitar Dinheiro/Tpa/Transferencia
        ...
        <div className="space-y-4 pb-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="p-5 rounded-xl flex-col sm:flex-row sm:items-center sm:justify-between gap-2"
                    style={{ background: 'color-mix(in srgb, var(--cor-aviso) 8%, transparent)', border: '2px solid var(--cor-aviso)' }}>
                    <div>
                ...
                <div className="p-5 rounded-xl flex-col sm:flex-row sm:items-center sm:justify-between gap-2"
                    style={{ background: 'color-mix(in srgb, var(--cor-sucesso) 4%, transparent)', border: '2px solid var(--cor-sucesso)' }}>
                    <div>
