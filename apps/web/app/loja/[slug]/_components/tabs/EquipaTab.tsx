...
import { UsuarioLoja } from "../modals/UserModal";

interface Props {
    equipa: UsuarioLoja[]; isAdmin: boolean; isDono: boolean;
    onAdd: () => void; onEdit: (u: UsuarioLoja) => void; onDelete: (u: UsuarioLoja) => void; onView: (u: UsuarioLoja) => void;
}
                    ...
                    <div key={u.id} className="flex flex-col sm:flex-row sm:items-center sm:justify-between p-3 sm:p-4 bg-neutral-800 rounded-lg gap-3">
                        ...
                        {isAdmin && (
                            <div className="flex gap-2 flex-wrap">
                                <Button size="sm" className="bg-blue-600 hover:bg-blue-700" onClick={() => onView(u)}><Eye size={12}/> Ver</Button>
                                {u.role!== 'DONO' && <Button size="sm" variant="secondary" onClick={() => onEdit(u)}>Editar</Button>}
