"use client";
import { Button } from "@/components/ui/button"; // <-- faltava esse
import { Dialog, DialogContent, DialogHeader, DialogTitle } from "@/components/ui/dialog";

export function DetalhesModal({ open, onClose, dados }: { open: boolean; onClose: () => void; dados: any }) {
    if (!dados) return null;
    return (
        <Dialog open={open} onOpenChange={onClose}> {/* <-- troquei pra onClose pra funcionar o ESC/click fora */}
            <DialogContent
                className="w-full sm:max-w-[700px] bg-neutral-900 border-white/10 text-white" // <-- corrigi w- e max-h-
            >
                <DialogHeader><DialogTitle>Detalhes: {dados.nome}</DialogTitle></DialogHeader>
                <div className="grid gap-4 py-4 max-h-[70vh] overflow-y-auto"> {/* <-- corrigi max-h- */}
