...
from api.app.models.venda import Venda # <- CORRIGIDO: Tirei ItemVenda daqui
from api.app.models.itens_venda import ItemVenda # <- CORRIGIDO: Import separado
...
async def criar_venda(db: AsyncSession, venda_in: VendaCreate, usuario: Usuario) -> VendaRead:
    async with db.begin():
        ...
        for item_in in venda_in.itens:
            ...
            itens_para_criar.append(ItemVenda( # <- CORRIGIDO: era.api.append
                produto_id=produto.id, 
                loja_id=usuario.lojas[0].id, # <- ADD: Faltava o loja_id
                quantidade=item_in.quantidade, 
                preco_unitario=produto.preco, 
                subtotal=subtotal
            ))

        nova_venda = Venda(loja_id=usuario.lojas[0].id, usuario_id=usuario.id, total=total_venda, itens=itens_para_criar)
        db.add(nova_venda)
        await db.flush()
...
async def listar_vendas(db: AsyncSession, usuario: Usuario, data_inicio: date | None = None, data_fim: date | None = None, vendedor_id: UUID | None = None) -> list[VendaRead]:
    ...
    if data_inicio: filtros.append(func.date(Venda.data_venda) >= data_inicio) # <- CORRIGIDO: era.api.append
    if data_fim: filtros.append(func.date(Venda.data_venda) <= data_fim) # <- CORRIGIDO: era.api.append
    if usuario.nivel == NivelUsuario.VENDEDOR: filtros.append(Venda.usuario_id == usuario.id)
...
async def estornar_venda_service(db: AsyncSession, venda_id: UUID):
    async with db.begin():
