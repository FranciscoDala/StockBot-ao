...
async def get_caixa_aberto(db: AsyncSession, loja_id: UUID) -> Caixa | None:
    ...
    stmt = select(Caixa).where(
        and_(
            ...
            Caixa.status == StatusCaixa.ABERTO # Agora funciona pq salvamos como VARCHAR
        )
    )
...
async def registrar_movimento_caixa(
    ...
    if not caixa: raise HTTPException(status_code=400, detail="Não é possível registrar: caixa fechado")

    mov = MovimentacaoCaixa(
        caixa_id=caixa.id, loja_id=loja_id, tipo=tipo, valor=valor, descricao=descricao,
...
async def get_resumo_caixa(loja_id: UUID, db: AsyncSession = Depends(get_db), current_user: Usuario = Depends(get_current_user)):
    ...
    return CaixaResumoOut(id=caixa.id, saldo_abertura=caixa.saldo_abertura, entradas_hoje=caixa.total_entradas, saidas_hoje=caixa.total_saidas, saldo_atual=caixa.saldo_esperado, status=caixa.status)
...
async def abrir_caixa(body: CaixaAbrirIn, db: AsyncSession = Depends(get_db), current_user: Usuario = Depends(get_current_user)):
    ...
    if await get_caixa_aberto(db, body.loja_id): raise HTTPException(status_code=400, detail="Já existe um caixa aberto para hoje")
    loja = await db.get(Loja, body.loja_id)
    if not loja: raise HTTPException(status_code=404, detail="Loja não encontrada")
    try:
        novo_caixa = Caixa(loja_id=body.loja_id, data_caixa=date.today(), data_abertura=datetime.utcnow(), usuario_abertura_id=current_user.id, saldo_abertura=body.saldo_abertura, saldo_esperado=body.saldo_abertura, status=StatusCaixa.ABERTO, observacao=body.observacao)
        db.add(novo_caixa)
        await db.flush()
        await registrar_movimento_caixa(db=db, loja_id=body.loja_id, tipo=TipoMovimentacao.ABERTURA, valor=body.saldo_abertura, descricao=f"Abertura de caixa: {body.saldo_abertura}", usuario_id=current_user.id)
        await db.commit()
...
async def fechar_caixa(caixa_id: UUID, body: CaixaFecharIn, db: AsyncSession = Depends(get_db), current_user: Usuario = Depends(get_current_user)):
    ...
    try:
        ...
        await registrar_movimento_caixa(db=db, loja_id=caixa.loja_id, tipo=TipoMovimentacao.SAIDA, valor=caixa.saldo_esperado, descricao="Fechamento de caixa", usuario_id=current_user.id)
        await db.commit()
...
async def fazer_sangria(body: SangriaIn, db: AsyncSession = Depends(get_db), current_user: Usuario = Depends(get_current_user)):
    ...
    if caixa.saldo_esperado < body.valor: raise HTTPException(status_code=400, detail="Saldo insuficiente para sangria")
    try:
        await registrar_movimento_caixa(db=db, loja_id=body.loja_id, tipo=TipoMovimentacao.SANGRIA, valor=body.valor, descricao=body.descricao, usuario_id=current_user.id)
        await db.commit()
    except Exception as e:
        await db.rollback()
